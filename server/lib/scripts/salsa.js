(function() {
  ...
  $('#salsa').after("<form id='salsa-form' class='mfp-hide white-popup-block'>" + "<h1>Login to Make Changes</h1>" + "<input id='salsa-email' name='email' type='text' placeholder='Salsa Username' />" + "<input id='salsa-password' name='password' type='password' placeholder='Salsa Password' />" + "<input type='submit' value='Log in' />" + "</form>");
  ...
  $('#salsa-form').submit(function(ev) {
    ...
    return $.getJSON(SERVER_NAME + '/signin?callback=?', data, function(data, status, xhr) {
      return alert(data);
    });
  });
